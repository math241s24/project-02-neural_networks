...
p4_no_layers <- plot_prediction(grid4_no_layers, spiral_data, "Model 3 Predictions")
p4_5node_layer <- plot_prediction(grid4_5node_layer, spiral_data, "Model 3 Predictions")
...
ui <- fluidPage(
  titlePanel("Basic Neural Networks Visualization"),
  sidebarLayout(
    sidebarPanel(
      selectInput("dataset", "Choose a dataset:", choices = c("Gaussian distribution", 
                                                             "Horizontal split", 
                                                             "Quadrants", 
                                                             "Spiral data")),
...
server <- function(input, output) {
  ...
  output$scatterPlot <- renderPlot({
     ...
     if (input$learningRate == "default" && input$HiddenLayers == "default")
        if (input$dataset == "Gaussian distribution") {
          p1
        } else if (input$dataset == "Horizontal split") {
          p2
        } else if (input$dataset == "Quadrants") {
          p3
        } else if (input$dataset == "Spiral data") {
          p4
        }
    else
        if  (input$HiddenLayers == "default")
          if (input$dataset == "Gaussian distribution") {
            model <- neuralnet(label~x+y, data=train1, hidden=c(5,5), learningrate=learning_rate,        linear.output=FALSE)
            grid <- generate_grid(data_two)
            grid$pred_label <- predict_grid(model, grid)$pred_label
            plot_prediction(grid, data_two, "Model Predictions with LR = " + toString(learning_rate) + ", Hidden Layers = " + toString(length(hidden_layers)))
    ...
    #need to update
            ...
            if (input$dataset == "Gaussian distribution") {
             model1 <- neuralnet(label~x+y, data=train1, hidden=c(5,5), learningrate=input$learningRate, linear.output=FALSE)
