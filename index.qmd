...
include_graphics("data/neuralnetwork.png")
```
...
```{r}
plot_grid(ggplot(data_two, aes(x=x, y=y, color=factor(label))) +
  geom_point(alpha=0.6) +
  labs(title="Classify Two Gauss Data", color="Label") +
  theme_minimal(), ggplot(data_by_x, aes(x=x, y=y, color=factor(label))) +
  geom_point(alpha=0.6) +
  labs(title="Data Classified by X Value", color="Label") +
  theme_minimal(), ggplot(data_by_xy, aes(x=x, y=y, color=factor(label))) +
  geom_point(alpha=0.6) +
  labs(title="Data Classified by XY Product", color="Label") +
  theme_minimal(), ggplot(spiral_data, aes(x=x, y=y, color=factor(label))) +
  geom_point(alpha=0.6) +
...
grid4_no_layers$pred_label <- predict_grid(model4_no_layers, grid4_no_layers)$pred_label
...
grid4_5node_layer$pred_label <- predict_grid(model4_5node_layer, grid4_5node_layer)$pred_label
...
p4 <- plot_prediction(grid4, spiral_data, "Model 1 Predictions")
p4
```
