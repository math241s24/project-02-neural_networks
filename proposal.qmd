...
create_model <- function() {
  model <- keras_model_sequential() %>%
    keras::layer_dense(units = 8, activation = 'relu', input_shape = c(2)) %>%
...
history1 <- model1 %>%
  fit(
    x = as.matrix(data_two[, 1:2]),
    y = as.matrix(data_two[, 3]),
    epochs = 100,
    batch_size = 10,
    validation_split = 0.2
  )
...
model2 <- create_model()
history2 <- model2 %>% fit(
  x = as.matrix(data_by_x[, 1:2]),
  y = as.matrix(data_by_x[, 3]),
  epochs = 100,
  batch_size = 10,
  validation_split = 0.2
)
...
model3 <- create_model()
history3 <- model3 %>% fit(
  x= as.matrix(data_by_xy[, 1:2]),
  y = as.matrix(data_by_xy[, 3]),
  epochs = 100,
  batch_size = 10,
  validation_split = 0.2
)
